...
public class LineChartView extends ChartView {
  ...
  /** Radius clickable region */
    ...
    int skip = set.getSkip();
		for(ChartEntry e : set.getEntries()) {
      if(skip>0){
        skip--;
        continue;
      } else {
        path.addCircle(e.getX(), e.getY(), set.getDotsRadius(), Path.Direction.CW);
      }
    }
    ...
    int skip = set.getSkip();
      ...
      if(skip>0){
        ...
        if (set.getEntry(i).getY() < minY) minY = set.getEntry(i).getY();
    ...
    int skip = set.getSkip();
		...
		final Path path = new Path();
	  path.moveTo(set.getEntry(skip).getX(),set.getEntry(skip).getY());

		final Path bgPath= new Path();
		bgPath.moveTo(set.getEntry(skip).getX(), set.getEntry(skip).getY());
